...
class CrossEntropyLoss(Loss):
    ...
    def forward(self, input, target):
        assert isinstance(input, pyflashlight.Tensor), \
            "Cross entropy argument 'input' must be Tensor, not {}".format(type(input))
        
        assert isinstance(target, pyflashlight.Tensor), \
        ...
        if input.ndim == 1:
            if target.numel == 1:
                ...
                target = pyflashlight.one_hot_encode(target, num_classes)
                
                logits = pyflashlight.softmax(input, dim=0)
            ...
            if target.ndim == 1:
                ...
                logits = pyflashlight.softmax(input, dim=1)
                target = target.reshape(logits.shape)
